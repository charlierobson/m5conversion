...
SndArray = $7020				;	DS	2		; music array pointer?
NoiseP = $7022					;	DS	2		; Pointer to program for noise generator
Tone1P = $7024					;	DS	2		; Pointer to program for tone1 generator
Tone2P = $7026					;	DS	2		; Pointer to program for tone2 generator
Tone3P = $7028					;	DS	2		; Pointer to program for tone3 generator
NoiseCtlShad = $702a			;	DS	1		; Noise control register shadow

Stack = $73b9					;	DS	1		; Default initial stack pointer
ParmArea = $73ba				;	DS	9		; Parameter storage for PCOPY parameters
VDP0Shad = $73c3				;	DS	1		; VDP register 0 shadow
VDP1Shad = $73c4				;	DS	1		; VDP register 1 shadow
VDPStatusByte = $73c5			;	DS	1
DeferWrites	= $73c6				;	DS	1		; flag byte?
WrtRAMSprt = $73c7				;	DS	1		; BlkWrtVRAM to RAM sprite attr table if =1
RandSeed = $73c8				;	DS	2		; Random number seed
D73CA = $73CA					;	DS	1		; unknown
D73CB = $73CB					;	DS	1		; unknown
D73CC = $73CC					;	DS	1		; unknown
D73CD = $73CD					;	DS	2		; unknown
D73CF = $73CF					;	DS	2		; unknown
D73D1 = $73D1					;	DS	2		; unknown
TimerList = $73D3				;	DS	2		; Pointer to timer list
TimerAux = $73d5				;	DS	2		; Pointer to end of timer aux storage
RawCtlState = $73d7				;	DS	20		; Raw controller state table (2 x 10 bytes)
PulseCnt1 = $73eb				;	DS	1		; Pulse counter #1
PulseCnt2 = $73ec				;	DS	1		; Pulse counter #2
Reserved73ed = $73ed			;	DS	1		; unused?
Joy1Shad = $73ee				;	DS	1		; shadow for joystick #1
Joy2Shad = $73ef				;	DS	1		; shadow for joystick #2
Key1Shad =  $73f0				;	DS	1		; shadow for keypad #1
Key2Shad =  $73f1				;	DS	1		; shadow for keypad #2
VDPBaseShad = $73f2				;	EQU	$		; shadow for VDP table base addresses
SprtTabShad = $73f2				;	DS	2		;   shadow for sprite table VRAM base addr
SprtPatTabShad = $73f4			;	DS	2		;   shadow for sprite pattern generator VRAM base addr
NameTabShad = $73f6				;	DS	2		;   shadow for name table VRAM base address
PatGenTabShad = $73f8			;	DS	2		;   shadow for pattern generator VRAM base addr
ClrTabShad = $73fa				;	DS	2		;   shadow for color table VRAM base address
SaveTemp = $73fc				;	DS	2		; unused?
D73FE = $73fe					;	DS	2		; temp3
...
IO_Sound		EQU	$20			; Sound chip output port
IO_VDP_Data		EQU	$10			; VDP data port
IO_VDP_Addr		EQU	$11			; VDP VRAM address output port
IO_VDP_Status	EQU	$11			; VDP status input port
...
;***************************************
ADDRFIX($0069)
	DB		60			; this might mean a 60hz display (NTSC)
ADDRFIX($006A)
	DW		D16AB		; this points to the font bitmap for 'A'
ADDRFIX($006C)
	DW		D1623		; this points to the font bitmap for '0'
...
D1ABF = $ ; DB '2'
	LD		(DeferWrites),A	; Clear some unknown flags
...
L1352		POP	HL		; End of list, clean up
	...
	LD	HL,BIOSSTART+$143B	; Point to color table
...
L02E6		CALL	L012F
	...
	RET	NZ
ADDRFIX($02EE)
	LD	A,(IX+00H)	; (called by Carnival)
	PUSH	AF
	CALL	L035F
	POP	BC
	LD	A,(IX+00H)
...
L04E7		CALL	L0572
	...
	LD	H,00H
	POP	AF
	RET	NC		; this is the carry flag from ACTIVATE
	PUSH	AF
...
L05CE		DEC	HL
	...
	DJNZ	L05CE
	PUSH	DE
	CALL	L1EA5		; LD A,04H LD IY,0001H JP PUT_VRAM
	POP	DE
	POP	BC
	INC	DE
	INC	BC
...
L0A54		POP	DE		; get rid of return address
	...
	LD	E,(IX+04H)
	POP	HL
	CALL	L1E92		; LD IY,0001H / JP GET_VRAM
	XOR	A
	POP	IY
	LD	(IY+01H),A
	LD	A,80H
...
InitScrn	XOR	A		; Fill VRAM from address 0000H
	...
	CALL	MODE_1	; Initialize the video chip
	JP	LOAD_ASCII	; Initialize the text font
...
L0BB8		SRA	A
	...
	ADD	HL,DE
	POP	DE
L0BD6		CALL	L1E90		; LD A,04H LD IY,0001H JP GET_VRAM
L0BD9		POP	BC
	POP	HL
	POP	DE
	DJNZ	L0B64
...
;***************************************
TIME_MGR	LD	HL,(TimerList)	; Get timer list pointer
...
;***************************************
INIT_TIMERP	LD	BC,P_InitTimers
	LD	DE,ParmArea
	CALL	PCopy
	LD	HL,(ParmArea)
	LD	DE,(ParmArea+2)
...
;***************************************
INIT_TIMER	LD	(TimerList),HL	; Store timer counter list pointer
...
;***************************************
FREE_SIGNALP	LD	BC,P_StopTimer
	LD	DE,ParmArea+4
	CALL	PCopy
	LD	A,(ParmArea+4)
...
;***************************************
FREE_SIGNAL	LD	HL,(TimerList)	; Get timer list pointer
	OR	A
...
;***************************************
REQUEST_SIGNALP	LD	BC,P_StartTimer
	LD	DE,ParmArea+5
	CALL	PCopy
	LD	HL,(ParmArea+6)
	LD	A,(ParmArea+5)
...
;***************************************
REQUEST_SIGNAL	LD	C,A		; C = timer number
...
; Parm 1 (byte) = event timer index (0=first timer)
;***************************************
TEST_SIGNALP	LD	BC,P_TestTimer
	LD	DE,ParmArea+8
	CALL	PCopy
	LD	A,(ParmArea+8)
...
;***************************************
TEST_SIGNAL	LD	C,A		; C = timer index
...
ADDRFIX($14C3)
	...
	.byte	$3F, $7F, $FF, $FF, $F3, $F3, $F0, $F0
	.byte	$00, $80, $C0, $C0, $C0, $C0, $00, $00
	.byte	$3F, $7F, $FF, $FF, $F3, $F3, $F3, $F3
	.byte	$00, $80, $C0, $C0, $C0, $C0, $C0, $C0
	.byte	$F0, $F0, $F0, $F0, $F0, $F0, $F0, $F0
	.byte	$FF, $FF, $FF, $F0, $F0, $FF, $FF, $FF
	.byte	$C0, $C0, $C0, $00, $00, $00, $00, $00
	.byte	$F1, $F1, $F1, $7B, $7B, $7B, $3F, $3F
	.byte	$E0, $E0, $E0, $C0, $C0, $C0, $80, $80
	.byte	$1F, $3F, $7F, $79, $78, $7F, $7F, $3F
	.byte	$80, $C0, $E0, $E0, $00, $80, $C0, $E0
	.byte	$F3, $F3, $FB, $FB, $FB, $FF, $FF, $FF
	.byte	$C0, $C0, $C0, $C0, $C0, $C0, $C0, $C0
	.byte	$F3, $F3, $FF, $FF, $7F, $3F, $00, $00
	.byte	$C0, $C0, $C0, $C0, $80, $00, $00, $00
	.byte	$F0, $F0, $FF, $FF, $FF, $FF, $00, $00
	.byte	$00, $00, $C0, $C0, $C0, $C0, $00, $00
	.byte	$3F, $1F, $1F, $1F, $0E, $0E, $00, $00
	.byte	$80, $00, $00, $00, $00, $00, $00, $00
	.byte	$F0, $F0, $F0, $F0, $F0, $F0, $00, $00
	.byte	$1F, $01, $79, $7F, $3F, $1F, $00, $00
	.byte	$E0, $E0, $E0, $E0, $C0, $80, $00, $00
	.byte	$FF, $F7, $F7, $F7, $F3, $F3, $00, $00
...
ADDRFIX($158B)
	...
	.byte	$1F, $04, $04, $04, $00, $00, $00, $00 ; T
	.byte	$44, $6C, $54, $54, $00, $00, $00, $00 ; M
...
ADDRFIX($15A3)
	...
	.byte	$08, $1C, $1C, $18, $10, $00, $30, $00 ; !
	.byte	$33, $33, $22, $00, $00, $00, $00, $00 ; "
	.byte	$36, $36, $7F, $36, $7F, $36, $36, $00 ; #
	.byte	$0C, $1F, $2C, $1E, $0D, $3E, $0C, $00 ; $
	.byte	$00, $63, $66, $0C, $18, $33, $63, $00 ; %
	.byte	$38, $64, $64, $38, $6D, $46, $3B, $00 ; &
	.byte	$18, $18, $30, $00, $00, $00, $00, $00 ; '
	.byte	$0C, $18, $30, $30, $30, $18, $0C, $00 ; (
	.byte	$18, $0C, $06, $06, $06, $0C, $18, $00 ; )
	.byte	$08, $6B, $3E, $1C, $3E, $6B, $08, $00 ; *
...
D1623
	.byte	$1C, $26, $63, $63, $63, $32, $1C, $00 ; 0
	.byte	$18, $38, $18, $18, $18, $18, $7E, $00 ; 1
	.byte	$3E, $63, $07, $1E, $3C, $70, $7F, $00 ; 2
	.byte	$3F, $06, $0C, $1E, $03, $63, $3E, $00 ; 3
	.byte	$0E, $1E, $36, $66, $7F, $06, $06, $00 ; 4
	.byte	$7E, $60, $7E, $03, $03, $63, $3E, $00 ; 5
	.byte	$1E, $30, $60, $7E, $63, $63, $3E, $00 ; 6
	.byte	$7F, $63, $06, $0C, $18, $18, $18, $00 ; 7
	.byte	$3C, $62, $72, $3C, $4F, $43, $3E, $00 ; 8
	...
	.byte	$3E, $63, $67, $6B, $6E, $60, $3E, $00 ; @
D16AB
	.byte	$1C, $36, $63, $63, $7F, $63, $63, $00 ; A
	.byte	$7E, $63, $63, $7E, $63, $63, $7E, $00 ; B
	.byte	$1E, $33, $60, $60, $60, $33, $1E, $00 ; C
	.byte	$7C, $66, $63, $63, $63, $66, $7C, $00 ; D
	.byte	$7F, $60, $60, $7E, $60, $60, $7F, $00 ; E
	.byte	$7F, $60, $60, $7E, $60, $60, $60, $00 ; F
	.byte	$1F, $30, $60, $67, $63, $33, $1F, $00 ; G
	.byte	$63, $63, $63, $7F, $63, $63, $63, $00 ; H
	.byte	$3F, $0C, $0C, $0C, $0C, $0C, $3F, $00 ; I
	.byte	$03, $03, $03, $03, $03, $63, $3E, $00 ; J
	.byte	$63, $66, $6C, $78, $7C, $6E, $67, $00 ; K
	.byte	$60, $60, $60, $60, $60, $60, $7F, $00 ; L
	.byte	$63, $77, $7F, $7F, $6B, $63, $63, $00 ; M
	.byte	$63, $73, $7B, $7F, $6F, $67, $63, $00 ; N
	.byte	$3E, $63, $63, $63, $63, $63, $3E, $00 ; O
	.byte	$7E, $63, $63, $63, $7E, $60, $60, $00 ; P
	.byte	$3E, $63, $63, $63, $6F, $66, $3D, $00 ; Q
	.byte	$7E, $63, $63, $67, $7C, $6E, $67, $00 ; R
	.byte	$3C, $66, $60, $3E, $03, $63, $3E, $00 ; S
	.byte	$3F, $0C, $0C, $0C, $0C, $0C, $0C, $00 ; T
	.byte	$63, $63, $63, $63, $63, $63, $3E, $00 ; U
	.byte	$63, $63, $63, $77, $3E, $1C, $08, $00 ; V
	.byte	$63, $63, $6B, $7F, $7F, $77, $63, $00 ; W
	.byte	$63, $77, $3E, $1C, $3E, $77, $63, $00 ; X
	.byte	$33, $33, $33, $1E, $0C, $0C, $0C, $00 ; Y
	.byte	$7F, $07, $0E, $1C, $38, $70, $7F, $00 ; Z
	.byte	$1E, $18, $18, $18, $18, $18, $1E, $00 ; [
	.byte	$00, $60, $30, $18, $0C, $06, $03, $00 ; \
	.byte	$3C, $0C, $0C, $0C, $0C, $0C, $3C, $00 ; ]
	.byte	$08, $1C, $36, $63, $00, $00, $00, $00 ; ^
	.byte	$00, $00, $00, $00, $00, $00, $7F, $00 ; _
	...
	.byte	$30, $5A, $0C, $00, $00, $00, $00, $00 ; ~
	.byte	$54, $28, $54, $28, $54, $28, $54, $00 ; del

;		Character cell to font data mapping for COLECOVISION

ADDRFIX($18A3)

	.byte	$01, $02, $0E, $0F, $08, $09, $12, $13 ; CV
	.byte	$03, $04, $0E, $0F, $05, $14, $00, $00 ; OI
	.byte	$05, $00, $10, $11, $0A, $0B, $15, $16 ; LS
	.byte	$06, $07, $10, $11, $05, $14, $00, $00 ; EI
	.byte	$01, $02, $0E, $0F, $03, $04, $0E, $0F ; CO
	.byte	$03, $04, $0E, $0F, $0C, $0D, $17, $18 ; ON
	.byte	$FF				; end of list
...
FILL_VRAM	LD	C,A		; Save fill byte
...
L18DD		LD	A,C		; Get back fill byte (and delay?)
	...
	JP	READ_REGISTER	; Read VDP status register
...
; entry into title screen waiter
ADDRFIX($1968)

	LD		HL,$1700 	; 1700H	is original startup screen delay timer
ADDRFIX($196B)
...
GAME_OPT	CALL	InitScrn	; Clear VRAM and initialize the VDP

	LD	BC,0F04H	; Reg 0FH (FG/BG color), value = 4
	CALL	WRITE_REGISTER	;   write it
...
;***************************************
UPDATE_SPINNER
	ret \ nop \ nop		;	LD	HL,PulseCnt1	; Point to first pulse counter
	nop \ nop			;		IN	A,(IO_Joy1)	; Read joystick 1
	CALL	L117D		; Do first pulse counter
	INC	HL		; Point to second pulse counter
	nop \ nop			;	IN	A,(IO_Joy2)	; Read joystick 2
...
INIT_TABLE	LD	C,A		; VDP register code to BC
	LD	B,00H
	LD	IX,VDPBaseShad
...
L1B72		JP	WRITE_REGISTER	; Write it to the VDP chip
...
PUT_VRAM	PUSH	AF  		; Save VDP table number
...
;***************************************
INIT_SPR_ORDERP	LD	BC,P_InitRAMSprt
	LD	DE,ParmArea
	CALL	PCopy
	LD	A,(ParmArea)
...
; Parm 1 (byte) = number of sprites
;***************************************
WR_SPR_NM_TBLP	LD	BC,P_CopyRAMSprt
	LD	DE,ParmArea
	CALL	PCopy
	LD	A,(ParmArea)
...
WR_SPR_NM_TBL	LD	IX,(RAMSprtIdx)	; Get pointer to RAM sprite index table
	PUSH	AF
...
WRITE_REGISTERP	LD	BC,P_WriteReg
	LD	DE,ParmArea
	CALL	PCopy
	LD	HL,(ParmArea)
	LD	C,H
...
WRITE_VRAM	LD	A,E		; Send LSB of address
	OUT	(IO_VDP_Addr),A
	LD	A,D
...
READ_VRAMP	LD	BC,$c05e ;P_ReadVRam
	LD	DE,ParmArea
	CALL	PCopy
	LD	HL,(ParmArea)
	LD	DE,(ParmArea+2)
...
doXpand	LD	HL,(VDP_Temp)	; HL = temp
	...
	CALL	PUT_VRAM	;   to the VRAM table
	CALL	L1E5D		; Check for hi-res graphics
...
L1E92		LD	IY,0001H	; Read one entry
	JP	GET_VRAM	;   from the color table
...
L1EA7		LD	IY,0001H	; Write one entry
	JP	PUT_VRAM	;   to the color table
...
;***************************************
DECODER	LD	A,L		; Check if reading keypad
	CP	01H
